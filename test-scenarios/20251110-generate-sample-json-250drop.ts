/**
 * Sample JSON Generator Script - 250mm Bracket Drop, No Notch
 *
 * Generates example JSON files for ShapeDiver integration using the
 * 5072mm @ 300mm centres test case with -250mm bracket drop and no notch.
 *
...
// ============================================================================
// Test Case Configuration (5072mm @ 300mm centres, -250mm drop, no notch)
// ============================================================================
...
const formInputs: FormDataType = {
  ...
  slab_thickness: 225,
  cavity: 100,
  support_level: -250, // bracket_drop - CHANGED from -50 to -250
  characteristic_load: 7.5,
  has_notch: false, // CHANGED from true to false
  notch_height: 0, // CHANGED from 60 to 0
  notch_depth: 0, // CHANGED from 22 to 0
...
const designInputs: DesignInputs = {
  support_level: 250, // positive value - CHANGED from 50 to 250
  cavity_width: 100,
  slab_thickness: 225,
  characteristic_load: 7.5,
...
async function generateSampleJSON() {
  ...
  console.log('Configuration: -250mm bracket drop, NO NOTCH\n')
  ...
  try {
    ...
    const bracketPath = join(process.cwd(), 'docs/json_tests/bracket_example_5072mm_300c_250drop_nonotch.json')
    ...
    const anglePath = join(process.cwd(), 'docs/json_tests/angle_example_5072mm_300c_250drop_nonotch.json')
    ...
    const runPath = join(process.cwd(), 'docs/json_tests/run_example_5072mm_300c_250drop_nonotch.json')
    ...
    console.log('   • Bracket drop: -250mm')
    console.log('   • Notch: DISABLED (no notch)')
    console.log('   • All other parameters same as original test case\n')
