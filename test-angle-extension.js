// For testing purposes, we'll need to use the compiled JavaScript or use ts-node
// Let's create a TypeScript version instead
...
async function testAngleExtension() {
    ...
    const config = {
        ...
        onProgress: (generation, maxGenerations, bestFitness) => {
            if (generation % 20 === 0) {
    ...
    try {
        ...
        console.log(`  Total Weight: ${result.result.calculated.weights?.totalWeight || 'NOT SET'} kg/m`);
