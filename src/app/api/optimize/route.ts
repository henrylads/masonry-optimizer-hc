...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (data.enable_fixing_optimization) {
      ...
      if (riseTobolts < 95) {
        const shortfall = 95 - riseTobolts;
        console.log('API route: Error - Insufficient rise to bolts:', riseTobolts);
        return NextResponse.json(
          { 
            error: 'Rise to bolts must be at least 95mm for structural safety',
            details: `Current rise to bolts: ${riseTobolts}mm (support level ${data.support_level}mm + fixing position ${fixingPosition}mm). Minimum required: 95mm. Shortfall: ${shortfall}mm.`,
            suggestion: `Increase the support level by at least ${shortfall}mm, or increase the fixing position by ${shortfall}mm (if within slab constraints).`
          },
          { status: 400 }
        );
      }
