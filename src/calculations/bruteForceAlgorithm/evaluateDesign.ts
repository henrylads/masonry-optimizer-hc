...
export function evaluateBruteForceDesign(
    ...
    // Geometric validation: Reject designs where the minimum bracket height constraint creates impossible geometry
    // This happens when the natural bracket height is less than 150mm but gets bumped up to meet the minimum.
    // The extra height has nowhere to go physically, resulting in invalid alternatives being shown.
    if (design.genetic.bracket_type === 'Standard' && design.genetic.fixing_position) {
        const Y = 40; // DISTANCE_FROM_TOP_TO_FIXING constant
        ...
        if (naturalBracketHeight < MIN_BRACKET_HEIGHT - 0.1) {
            console.log(`⚠️  Rejecting design: fixing@${design.genetic.fixing_position}mm produces natural height ${naturalBracketHeight.toFixed(1)}mm, below minimum ${MIN_BRACKET_HEIGHT}mm`);
            console.log(`    This creates impossible geometry where the bracket has ${(MIN_BRACKET_HEIGHT - naturalBracketHeight).toFixed(1)}mm of extra length with nowhere to go`);
            ...
            return {
                ...
                verificationResults: {
                    passes: false,
                    momentResults: { passes: false },
                    shearResults: { passes: false },
                    deflectionResults: { passes: false },
                    angleToBracketResults: { passes: false },
                    fixingResults: { passes: false },
                    bracketDesignResults: { passes: false },
                    combinedResults: { passes: false }
