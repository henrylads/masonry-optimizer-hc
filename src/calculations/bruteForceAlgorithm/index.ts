...
export async function runBruteForce(
    ...
    // Include calculated dimensions to prevent showing alternatives that have different genetic
    // parameters but produce identical geometry (e.g., different fixing positions that both
    // hit the 150mm minimum bracket height)
    const signature = (d: Design | { genetic: { bracket_centres: number; bracket_thickness: number; angle_thickness: number; bolt_diameter: number; bracket_type: string; angle_orientation: string; channel_type?: string; steel_bolt_size?: string; steel_fixing_method?: string }; calculated?: CalculatedParameters }) => {
        const geneticSig = [
        ...
        // Also include key calculated dimensions to distinguish designs
        // This prevents showing alternatives with different fixing positions but identical geometry
        if ('calculated' in d && d.calculated) {
            const calculatedSig = [
                d.calculated.bracket_height?.toFixed(0) || '0',
                d.calculated.bracket_projection?.toFixed(0) || '0',
                d.calculated.rise_to_bolts?.toFixed(0) || '0',
                d.calculated.vertical_leg || d.genetic.vertical_leg || '0'
            ].join('|');
            return `${geneticSig}|${calculatedSig}`;
        }
