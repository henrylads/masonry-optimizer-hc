...
export const BRACKET_ANGLE_CONSTANTS = {
  ...
  INVERTED_BRACKET_DEFAULT_DIM_D: 135, // mm
  /** Minimum Dim D for inverted brackets */
  INVERTED_BRACKET_MIN_DIM_D: 135, // mm
  ...
  INVERTED_BRACKET_MIN_HEIGHT: 175, // mm (135 + 40, aligned with min Dim D)

  // Standard bracket specific constants
  /** Minimum total height for standard brackets (structural minimum) */
 ...
 *
 * Based on the Bracket and Angle Selection Table in projectOverview.md:
 * - Inverted bracket: When BSL > -75mm (above the fixing point at -75mm from SSL)
 * - Standard bracket: When BSL ≤ -75mm (at or below the fixing point)
 *
...
export function getValidAngleOrientations(supportLevel: number): AngleOrientation[] {
  // Selection table logic (CORRECTED):
  // ≥ 0mm: Both (Standard, Inverted)
  // -25mm to -50mm: Inverted only (corrected - angle below SSL needs vertical leg extending down)
  ...
  } else if (supportLevel >= -50 && supportLevel < 0) {
    return ['Inverted']; // Inverted only (angle below SSL, vertical leg extends down to bracket)
...
export function calculateRiseToBolts(params: RiseToBoltsCalculationParams): number {
  ...
  console.log(`🔧 RISE TO BOLTS CALCULATION DEBUG:`, {
    bracket_height,
    distance_from_top_to_fixing,
    worst_case_adjustment,
    fixing_position,
  ...
  if (bracketType === 'Standard') {
    ...
    // Additional constraint for brackets that would project below slab
    const wouldProjectBelowSlab = requiredRiseToBoltsFromBracket > maxRiseToboltsFromSlabGeometry;

    if (wouldProjectBelowSlab) {
      console.log(`🔧 BRACKET PROJECTS BELOW SLAB - Limiting rise to bolts`);
      // Further limit based on bottom critical edge if bracket projects below slab
      actualRiseToBolts = Math.min(actualRiseToBolts, bottom_critical_edge_distance - worst_case_adjustment);
    }

    console.log(`🔧 STANDARD BRACKET - FINAL RISE TO BOLTS: ${actualRiseToBolts}mm (projects below slab: ${wouldProjectBelowSlab})`);
  } else {
    ...
    console.log(`🔧 INVERTED BRACKET - requiredRiseToBoltsToSupportLevel: ${requiredRiseToBoltsToSupportLevel}mm`);

    // The actual rise to bolts is the minimum of:
    // 1. What the bracket geometry allows (bracket height - top clearances)
    // 2. What the slab geometry allows (space to slab bottom - worst case adjustment)
    // 3. What's needed to reach the support level (inverted brackets only)
    actualRiseToBolts = Math.min(
      requiredRiseToBoltsFromBracket,
      maxRiseToboltsFromSlabGeometry,
    ...
    console.log(`🔧 INVERTED BRACKET - FINAL RISE TO BOLTS: ${actualRiseToBolts}mm (projects below slab: ${wouldProjectBelowSlab})`);
  }

  return roundToTwelveDecimals(Math.max(0, actualRiseToBolts));
}
