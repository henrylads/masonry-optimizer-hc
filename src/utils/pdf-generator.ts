...
export class CalculationPDFGenerator {
   ...
   */
  private addProjectInfoSection(): void {
    // Only display if there's project info available
    const projectInfo = this.data.metadata.projectInfo;
    if (!projectInfo) {
      return;
    }

    // Check if any project info fields are filled
    const hasProjectInfo = Object.values(projectInfo).some(value => value && value.trim().length > 0);
    if (!hasProjectInfo) {
      return;
    }
    ...
    // Create array of project info fields to display
    const projectFields: { label: string; value: string }[] = [];

    if (projectInfo.project_name) {
      projectFields.push({ label: 'Project Name', value: projectInfo.project_name });
    }
    if (projectInfo.section_name) {
      projectFields.push({ label: 'Section/Element', value: projectInfo.section_name });
    }
    if (projectInfo.client_name) {
      projectFields.push({ label: 'Client', value: projectInfo.client_name });
    }
    if (projectInfo.project_location) {
      projectFields.push({ label: 'Location', value: projectInfo.project_location });
    }
    if (projectInfo.project_reference) {
      projectFields.push({ label: 'Project Reference', value: projectInfo.project_reference });
    }
    if (projectInfo.designer_name) {
      projectFields.push({ label: 'Designer', value: projectInfo.designer_name });
    }
    ...
    projectFields.forEach(field => {
      ...
      this.doc.setFont(undefined, 'normal');
      this.doc.text(field.value, this.margin + 65, this.currentY);
      this.currentY += 7;
    });
